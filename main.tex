...
\begin{itemize}
  \item Bestrahlungsmaschine für Krebspatienten verrechnete sich 2000 systematisch bei der Dosierung
...
\frame{
	...
	\begin{block}{Funktionalität}
		\begin{itemize}
			\item Wie wird die gewünschte Funktionalität der Software geliefert? 
			\item Ist funktionale Korrektheit gegeben?
			\item Sind die Softwarefunktionen vollständig?
			\item Liegt angemessene Funktionalität vor?
		\end{itemize}
	\end{block}
}
...
\frame{
	\frametitle{Hauptqualitätsmerkmale von Software}
	\begin{block}{Effizienz/Performanz}
		\begin{itemize}
			\item Wie effizient ist das System in Relation zu dessen Ressourcenverbrauch? 
			\item Beweist das System ein gutes Zeitverhalten?
			\item Nutzt es Ressourcen effektiv?
			\item Schont es Kapazitäten?
		\end{itemize}
	\end{block}
}
...
\frame{
	...
	\begin{block}{Kompatibilität}
		\begin{itemize}
			\item Wie kompatibel ist das System mit anderen Systemen auf anderer oder gleicher Hardware? 
			\item Ist die Co-Existenz zu anderer Software optimal?
			\item Ist das System interoperabel?
		\end{itemize}
	\end{block}
}
...
\frame{
	\frametitle{Hauptqualitätsmerkmale von Software}
	\begin{block}{Benutzbarkeit}
		\begin{itemize}
			\item Wie gut lassen sich Ziele mit der Software für den Benutzer erreichen?
			\item Ist das System leicht erlernbar?
			\item Ist es gut bedienbar?
			\item Ist es vor Nutzerfehlern geschützt?
			\item Ist das User-Interface ansprechend?
		\end{itemize}
	\end{block}
}
...
\frame{
	...
	\begin{block}{Zuverlässigkeit}
		\begin{itemize}
			\item Wie zuverlässig ist die Leistung des Systems? 
			\item Ist die Software ausgereift?
			\item Weist sie eine Fehlertoleranz auf?
			\item Ist sie wiederherstellbar?
		\end{itemize}
	\end{block}
}
...
\frame{
	...
	\begin{block}{IT-Sicherheit}
		\begin{itemize}
			\item Wie schützt das System Daten? 
			\item Ist die Software imstande Datensicherheit zu gewährleisten?
			\item Ist das System integer?
			\item Ist das System vor Manipulation geschützt?
		\end{itemize}
	\end{block}
}
...
\frame{
	...
	\begin{block}{Wartbarkeit}
		\begin{itemize}
			\item Wie anpassungsfähig ist das System in Hinblick auf Fehlerkorrektur oder Veränderbarkeit als Reaktion auf dessen Umwelt oder sich wandelnde Rahmenbedingungen? 
			\item Ist die Software modular aufgebaut?
			\item Besteht sie aus wiederverwendbaren Komponenten?
			\item Ist sie leicht modifizierbar?
		\end{itemize}
	\end{block}
}
...
\frame{
	...
	\begin{block}{Übertragbarkeit}
		\begin{itemize}
			\item Wie übertragbar ist die Software? 
			\item Kann die Software von einer Umgebung in eine andere übertragen werden?
			\item Ist sie leicht zu installieren?
			\item Ist sie leicht austauschbar?
		\end{itemize}
	\end{block}
}
...
\node at (5.75,-1.75) [text width=7cm,fill=ivory, rounded corners=8pt] {int divident = 5;\\int divisor = 0;\\double result = dividend / divisor;};
...
\begin{block}{Definition}
Testverfahren, bei dem Testfälle im Hinblick auf die Ausführung verschiedener Verhalten eines Anwendungsfalls entworfen werden.
\end{block}
}
